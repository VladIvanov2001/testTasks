...
export class ActionWithBoard {
    ...
    getUnitByLocation(boardLocation: boardLocation): unit {
        if (boardLocation) {
            return this.gameBoard.getBoard()[boardLocation.rowNumber][boardLocation.columnNumber];
        }
        return null;
    }

    getUnitCondition(boardLocation: boardLocation): boolean {
        return Boolean(this.gameBoard.getBoard()[boardLocation.rowNumber][boardLocation.columnNumber]);
    }
    ...
    getUnitTeam(unitBoardLocation: boardLocation): Team {
        return unitBoardLocation.rowNumber < Math.floor(this.gameBoard.getBoard().length / 2)
            ? Team.redTeam : Team.orangeTeam;
    }
    ...
    getEnemyNeighborLocation(unitBoardLocation: boardLocation) {
        ...
        const isNextLineWithEnemy: boolean = this.getUnitTeam({
            rowNumber: unitBoardLocation.rowNumber + indexDirection,
            columnNumber: unitBoardLocation.columnNumber,
        }) !== team;

        const isLeftEnemy: boolean = isNextLineWithEnemy && unitBoardLocation.columnNumber - 1 >= 0;
        const isRightEnemy: boolean = isNextLineWithEnemy && unitBoardLocation.columnNumber + 1 <= this.gameBoard.getBoard().length;
        ...
        if (isNextLineWithEnemy && isLeftEnemy && this.getUnitCondition(enemyLeftLocation)) {
            neighborUnitLocation.push(enemyLeftLocation);
        }
        if (isNextLineWithEnemy && isRightEnemy && this.getUnitCondition(enemyRightLocation)) {
    ...
    getEnemyRow(row: number): (boardLocation | null)[] {
        return this.gameBoard.getBoard()[row].filter((unit) => {
            // @ts-ignore
            unit.hp > 0
        }).map((unit) => {
    ...
    getNextLineTeam(unitBoardLocation: boardLocation): Team | null {
        ...
        if (this.getEnemyRow(unitBoardLocation.rowNumber + indexDirection).length === 0) {
            return null;
        }
    ...
    getAllTeamUnits(unitBoardLocation: boardLocation, enemies = false): boardLocation[] {
        ...
        if (consideringTeam === Team.orangeTeam) {
            for (let i = Math.floor(gameBoard.length / 2); i >= 0; i--) {
                for (let j = 0; j < gameBoard[i].length; j++)
                    if (gameBoard[i][j]) {
                        const enemyBoardLocation = this.getUnitLocation(gameBoard[i][j] as Unit)
            ...
            for (let i = Math.floor(gameBoard.length / 2); i <= gameBoard.length; i++) {
                for (let j = 0; j < gameBoard[i].length; j++)
                    if (gameBoard[i][j]) {
                        const enemyBoardLocation = this.getUnitLocation(gameBoard[i][j] as Unit)
                        if (enemyBoardLocation) {
