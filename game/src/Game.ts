...
export class Game {
    ...
    static start(rowsNumber: number, columnsNumber: number): initialGameData {
        ...
        gameBoard.putAllUnitsOnBoard(randomizer.generateGameBoard(rowsNumber, columnsNumber));
        ...
        const unitAction = new UnitAction(this.actionWithBoard, gameBoard, this.switchQueue);
    ...
    static finish(currentUnit: Unit): { isFinished: boolean; currentTeam: Team } {
        return {
            isFinished: !this.actionWithBoard
                .getAllEnemiesLocation(this.actionWithBoard.getUnitLocation(currentUnit) as boardLocation)
                .some((enemyLocation) => this.actionWithBoard.getUnitCondition(enemyLocation)),
            currentTeam: this.actionWithBoard.getUnitTeam(this.actionWithBoard.getUnitLocation(currentUnit) as boardLocation),
        };
    }
}
